<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.$watch('details.registrantId', (newVal) => {
                    if (!newVal || newVal === "") {
                        this.validations.errors.registrant_id = [];
                        this.validations.errors.registrant_id.push(
            ...
            this.$watch('details.description', (newVal) => {
                    if (newVal == "") {
                        ...
                        this.validations.errors.description.push(
                            "The Registrant description is required."
                        );
                    } else if (newVal.length > 50) {
                        this.validations.errors.description = [];
                        this.validations.errors.description.push(
                            "The Registrant description must not exceed 50 characters."
                        );
                    } else if (this.letterOnly(newVal) == false ) {
                        this.validations.errors.description = [];
                        this.validations.errors.description.push(
                            "The Registrant must not contain any special characters."
                        );
                    } 
                    else {
                        this.validations.errors.description = [];
